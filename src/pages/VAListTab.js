...
export default function VAList({ custId }) {
    ...
    function onDatePickerChange(date, dateString) {
        const newData = data.filter(item => moment(item.created_at.substring(0, 10)).isSame(date, 'date'))
        setFiltered(newData)
    }
    ...
    return (
        <div className="top-space">
            ...
            <Row>
                <Col span={2}></Col>
                <Col span={20}>
                    <DataTable columns={columns} data={dataTable} pagePosition="bottomRight" />
                </Col>
                <Col span={2}></Col>
            </Row>
        </div>
